/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.liverpool.automatizacion.properties;

/**
 *
 * @author aperezg03
 */
public class Checkout_Paso4 {
    public static final String PROPERTIES_FILE = "Paso4_Checkout.properties";
    
    public static final String URLTICKET = "urlTicket";
    public static final String CLIENTE = "cliente";
    public static final String CORREOCLIENTE = "correoCliente";
    public static final String FECHA = "fecha";
    public static final String SKU = "sku";
    public static final String CANTIDADSKU = "cantidadSKU";
    public static final String FACTURACION = "facturacion";
    public static final String BOLETA = "boleta";
    public static final String TERMINAL = "terminal";
    public static final String TIENDA = "tienda";
    public static final String PEDIDO = "pedido";
    public static final String AUTOBANCARIA = "autoBancaria";
    public static final String PRECIOSKU = "precioSKU";
    public static final String CALLE = "calle";
    public static final String NEXTERIOR = "nExterior";
    public static final String NINTERIOR = "nInterior";
    public static final String COLONIA = "colonia";
    public static final String CP = "cp";
    public static final String DELEGACION = "delegacion";
    public static final String CIUDAD = "ciudad";
    public static final String FOLIOPAGO = "folioPago";
    public static final String FOLIOPAYPAL = "folioPaypal";
    public static final String TARJETA = "tarjeta";
    public static final String NTARJETA = "nTarjeta";
    public static final String FECHATARJETA = "fechaTarjeta";
    public static final String PRECIOTOTAL = "precioTotal";
    public static final String REFENCIACIE = "refenciaCIE";
    public static final String MESA = "mesa";
    public static final String FESTEJADO = "festejado";
    public static final String LOGOLIVERPOOL = "logoLiverpool";
}
